"use strict";
(self["webpackChunkquickso_cms"] = self["webpackChunkquickso_cms"] || []).push([[9298],{
...
var src_permissions = __webpack_require__(84060);
...
const fetchUser = (id) => __async(void 0, null, function* () {
  const { data } = yield core_utils/* axiosInstance */.SP.get(`/admin/users/${id}`);
  return data.data;
});
const putUser = (id, body) => __async(void 0, null, function* () {
  const { data } = yield core_utils/* axiosInstance */.SP.put(`/admin/users/${id}`, body);
  return data.data;
});
...
var users = __webpack_require__(66284);
...
const ProtectedEditPage = () => {
  ...
  const permissions = (0,react.useMemo)(() => {
    return {
      read: src_permissions/* default */.A.settings.users.read,
      update: src_permissions/* default */.A.settings.users.update
    };
  }, []);
  const {
    isLoading,
...
const commonUserSchema = {
  firstname: lib/* string */.Yj().trim().required(build.translatedErrors.required),
  lastname: lib/* string */.Yj(),
  email: lib/* string */.Yj().email(build.translatedErrors.email).lowercase().required(build.translatedErrors.required),
  username: lib/* string */.Yj().nullable(),
  password: lib/* string */.Yj().min(8, build.translatedErrors.minLength).matches(/[a-z]/, "components.Input.error.contain.lowercase").matches(/[A-Z]/, "components.Input.error.contain.uppercase").matches(/\d/, "components.Input.error.contain.number"),
  confirmPassword: lib/* string */.Yj().min(8, build.translatedErrors.minLength).oneOf([lib/* ref */.KR("password"), null], "components.Input.error.password.noMatch").when("password", (password, passSchema) => {
...
const schema = __spreadProps(__spreadValues({}, commonUserSchema), {
  currentPassword: lib/* string */.Yj().when(["password", "confirmPassword"], (password, confirmPassword, passSchema) => {
    return password || confirmPassword ? passSchema.required(build.translatedErrors.required) : passSchema;
  }),
  preferedLanguage: lib/* string */.Yj().nullable()
});
...
const roles_schema = {
  roles: lib/* array */.YO().min(1, build.translatedErrors.required).required(build.translatedErrors.required)
};
...
const edit_schema = lib/* object */.Ik().shape(edit_spreadValues(edit_spreadProps(edit_spreadValues({}, commonUserSchema), {
  isActive: lib/* bool */.lc()
}), roles));
